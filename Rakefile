...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/hfpp2012/acts_as_avatar/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("acts_as_avatar.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in acts_as_avatar.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "acts_as_avatar.gemspec", /ruby_version = .*">= (.*)"/ => lowest
